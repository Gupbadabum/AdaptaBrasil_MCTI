#!/usr/bin/env Rscript
# Script: calculo_probabilidade_frequencia.R
# Descrição: Calcula probabilidades de eventos climáticos extremos para municípios brasileiros
#             durante períodos de safra de milho (1ª e 2ª safra), com tratamento especial
#             para Fernando de Noronha (sem dados climáticos)
...
required_packages <- c("magrittr", "raster", "sf", "ncdf4")
...
OUTPUT_DIR <- file.path(BASE_DIR, "4-Output", REC)
...
process_municipality <- function(munic_code, munic_uf, nc_list, p1_safra, p2_safra) {
  ...
  Decendios <- substr(Dates[Dates <= '1995-12-31'], 6, 10)
...
tryCatch({
  ...
  message("Carregando dados climáticos...")
  nc_files <- list(
    count_5mm_6days = paste0('Count_5mm_6days_', DATA_PERIOD, '.nc'),
    count_15mm_3days = paste0('Count_15mm_3days_', DATA_PERIOD, '.nc'),
    count_20mm = paste0('Count_20mm_', DATA_PERIOD, '.nc'),
    count_50mm = paste0('Count_50mm_', DATA_PERIOD, '.nc'),
    count_cwd_5mm_6days = paste0('Count_cwd_5mm_6days_', DATA_PERIOD, '.nc'),
    count_cwd_10mm_3days = paste0('Count_cwd_10mm_3days_', DATA_PERIOD, '.nc')
  )
  ...
  message("Processando municípios...")
  total_munic <- nrow(map0_df)
  skipped_munic <- 0
  skipped_codes <- c(FERNANDO_NORONHA_CODE) # Pode ser expandido para outros códigos se necessário
  ...
  for (i in 1:total_munic) {
    ...
    munic_results <- process_municipality(munic_code, munic_uf, nc_crops, p1_safra, p2_safra)
  ...
  output_files <- list(
    count_5mm_6days = paste0('Prob_Count_5mm_6days_', DATA_PERIOD, '.csv'),
    count_15mm_3days = paste0('Prob_Count_15mm_3days_', DATA_PERIOD, '.csv'),
    count_20mm = paste0('Prob_Count_20mm_', DATA_PERIOD, '.csv'),
    count_50mm = paste0('Prob_Count_50mm_', DATA_PERIOD, '.csv'),
    count_cwd_5mm_6days = paste0('Prob_Count_cwd_5mm_6days_', DATA_PERIOD, '.csv'),
    count_cwd_10mm_3days = paste0('Prob_Count_cwd_10mm_3days_', DATA_PERIOD, '.csv')
  )
