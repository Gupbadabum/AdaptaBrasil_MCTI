...
###############################################################################
# SCRIPT: calculo_indicadores_cenarios_585.sh
# DESCRIÇÃO: Calcula indicadores climáticos de precipitação para cenário SSP5-8.5
# PERÍODO: 2066-2086
# MODELO: NorESM2-MM
# SAÍDAS:
...
SWL='3'
SSP='SSP585'
MODEL='NorESM2-MM-pr-ssp585'

# Caminhos de entrada/saída
PATH_IN="/home/usuario/Downloads/NC/CLIMBra/pr/ssp585/$MODEL.nc"
PATH_OUT="/home/usuario/Documentos/AdaptaBrasil/Seg_Alimentar/IC_chuva/4-Output/$SSP/"
...
process_indicador() {
    ...
    local extra_flags=$6   # Flags extras (como del29feb)

    echo "Processando ${tipo} para período ${data_ini} a ${data_end}"
    
    cdo $CDO_FLAGS -settaxis,"$data_ini" $operadores $extra_flags -seldate,"$data_ini","$data_end" "$PATH_IN" "$arq_saida"
...
process_decendio() {
    ...
    local base_name="${MODEL}_${SWL}_tmp_$ano-$mes-$sufixo"
...
consolidar_arquivos() {
    ...
    local arq_saida="${PATH_OUT}${tipo}/${MODEL}_${SWL}_Count_${padrao}_${ANO_INICIAL}_${ANO_FINAL}.nc"
    local arq_mean="${PATH_OUT}${tipo}/${MODEL}_${SWL}_Count_${padrao}_mean_${ANO_INICIAL}_${ANO_FINAL}.nc"
    
    cdo $CDO_FLAGS -mergetime "${PATH_OUT}${tipo}/${MODEL}_${SWL}_tmp_"*".nc" "$arq_saida"
    
    if [ $? -eq 0 ]; then
        rm -f "${PATH_OUT}${tipo}/${MODEL}_${SWL}_tmp_"*".nc"
...
echo "Iniciando processamento para cenário ${SSP} (${ANO_INICIAL}-${ANO_FINAL})..."
echo "Modelo: ${MODEL}"
echo "Arquivo de entrada: ${PATH_IN}"
echo "Diretório de saída: ${PATH_OUT}"
...
echo "Processamento concluído com sucesso para cenário ${SSP}!"
exit 0
