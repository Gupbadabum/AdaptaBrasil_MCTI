#!/usr/bin/env Rscript
# Script: calculo_probabilidade_frequencia_ssp245.R
# Descrição: Calcula probabilidades de eventos climáticos extremos para cenário futuro SSP245
#            utilizando modelo NorESM2-MM (2068-2088), com tratamento especial para Fernando de Noronha
...
PERIOD <- "2068_2088"
SWL <- "2" # Nível de aquecimento global
...
tryCatch({
  ...
  message("Carregando dados climáticos do modelo ", CLIMATE_MODEL, "...")
  nc_files <- list(
    count_5mm_6days = paste0(CLIMATE_MODEL, '-pr-', tolower(SCENARIO), '_', SWL, '_Count_5mm_6days_', PERIOD, '.nc'),
    count_15mm_3days = paste0(CLIMATE_MODEL, '-pr-', tolower(SCENARIO), '_', SWL, '_Count_15mm_3days_', PERIOD, '.nc'),
    count_20mm = paste0(CLIMATE_MODEL, '-pr-', tolower(SCENARIO), '_', SWL, '_Count_20mm_', PERIOD, '.nc'),
    count_50mm = paste0(CLIMATE_MODEL, '-pr-', tolower(SCENARIO), '_', SWL, '_Count_50mm_', PERIOD, '.nc'),
    count_cwd_5mm_6days = paste0(CLIMATE_MODEL, '-pr-', tolower(SCENARIO), '_', SWL, '_Count_cwd_5mm_6days_', PERIOD, '.nc'),
    count_cwd_10mm_3days = paste0(CLIMATE_MODEL, '-pr-', tolower(SCENARIO), '_', SWL, '_Count_cwd_10mm_3days_', PERIOD, '.nc')
  )
  
  nc_list <- lapply(nc_files, function(f) {
    file_path <- file.path(OUTPUT_DIR, dirname(f), basename(f))
    if (!file.exists(file_path)) {
  ...
  message("Processando municípios para o cenário ", SCENARIO, "...")
